...
export const useAgora = (channelName: string) => {
     ...
     const startAudioMixing = async (url) => {
          if (Platform.OS === "android" || Platform.OS === "ios") {
               agoraEngineRef.current?.startAudioMixing(
                    url, // Path to the audio file (local or remote)
                    false, // Whether to play the music file only locally
                    // false, // Whether to replace the original audio with the music file
                    1, // Number of times to loop the audio file (0 means infinite loop)
               );
          }
     };
